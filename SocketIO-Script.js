...
                nsp: '/'
            };
            // Relying on the global msgpack library loaded by the game or another script
            const encoded = msgpack.encode(packet);
            unsafeWindow._MM_sendRaw(encoded);
        } catch (e) {
            console.error("[ModMenu] Packet send failed. Is msgpack loaded?", e);
        }
    };
...
    function initMenu() {
        // Prevent double injection if it somehow fires twice
        if (document.querySelector('.mm-box')) return;
...
            .checkbox("Speed Hack", "speed_hack_enabled", true, val => { if(typeof unsafeWindow._MM_Log !== 'undefined') unsafeWindow._MM_Log("Speed hack:", val); })
            .checkbox("No-Clip", "noclip_enabled", false, val => { if(typeof unsafeWindow._MM_Log !== 'undefined') unsafeWindow._MM_Log("No clip:", val); });

        general.section("Economy Setup", "Money injections")
            .button("💸 Inject 1000 Coins", "green", () => {
                if(typeof unsafeWindow._MM_Log !== 'undefined') unsafeWindow._MM_Log("Joined room 901");
                sendPacket('join_room', { room: 901, x: 100, y: 100 });
                setTimeout(() => {
                    sendPacket('game_over', { coins: 1000 });
                }, 5000);
                if(typeof unsafeWindow._MM_Log !== 'undefined') unsafeWindow._MM_Log("Sent gameover with 1000 coins");
            });
...
            .button("🔍 Dump Sockets", "blue", () => { if(typeof unsafeWindow._MM_Log !== 'undefined') unsafeWindow._MM_Log('[ModMenu] Sockets:', unsafeWindow._MM_SOCKETS.length); })
...
                            });
                            if(typeof unsafeWindow._MM_Log !== 'undefined') unsafeWindow._MM_Log(`Successfully compiled ${parsedRoomsArray.length} rooms natively.`);

                        } catch (err) {
                            if(typeof unsafeWindow._MM_Error !== 'undefined') unsafeWindow._MM_Error("Failed to parse game crumbs data.", err);
                        }
                    }
                }
            });
...
    // 🚨 THIS IS THE FIX: Force the menu to spawn INSTANTLY no matter what the socket is doing.
    if (document.body) {
        initMenu();
    } else {
...
    // Now we just run the status check quietly in the background without breaking the UI injection
...
        
        // Wrap this in a try/catch so it doesn't shit the bed if unsafeWindow isn't fully loaded yet
        let isSocketActive = false;
        try {
            isSocketActive = !!(typeof unsafeWindow !== 'undefined' && unsafeWindow._MM_SOCKETS && unsafeWindow._MM_SOCKETS.length > 0);
        } catch (e) {}
